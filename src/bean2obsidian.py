...
class Bean2Obsidian:
    def __init__(self, bean_file: str, obsidian_folder: str, bean_conqueror_folder: Optional[str] = None):
        ...
        self.coffee_folder = self.obsidian_folder.joinpath("002-literature", "004-coffee")
    ...
    def beans(self):
        ...
        for name, bean in self.bean_data.beans.beans.items():
            ...
            product_path = bean_path.joinpath(example_bean_object.roaster, example_bean_object.name)
            ...
            bean_note.set_frontmatter("coffeeIsSingleOrigin", example_bean_object.is_single_origin())
            bean_note.set_frontmatter("coffeeBeanAromatics", example_bean_object.aromatics)
            bean_note.set_frontmatter("coffeeBeanRoastLevel", example_bean_object.roasting_level)
            ...
            bean_note.add_content('`$=await dv.view("902-template/002-dataview/coffeeBeanView", {})`')
            bean_notes.append(
                {
                    "note": bean_note,
                    "path": note_path
            ...
            for history in bean.get("history", []):
                ...
                history_note.set_frontmatter("price", f"£{round((float(history.price)/float(history.weight))*100, 2)}/100g")
    ...
    def brews(self):
        ...
        for brew in self.bean_data.brews.brews:
            ...
            zettel_note.set_frontmatter('coffeeGrinder', f"[[Coffee Grinder - {mill.name}|{mill.name}]]")
            # Bean Link
            bean = self.bean_data.beans.get_bean_by_uuid(brew.bean)
            zettel_note.set_frontmatter("coffeeBean", f"[[Coffee Purchase Record - {brew.bean}|{bean.name}]]")
            # Preparation Link
            preparation = self.bean_data.preparations.get_preparation_by_uuid(brew.preparation)
            tools = brew.tool_of_preparation
            methods = []
            if len(tools) > 0:
                for tool in tools:
                    tool_name = self.bean_data.preparations.get_preparation_tool_by_uuid(tool)
                    methods.append(f"[[Coffee Preparation Tool - {tool_name.uuid}|{tool_name.name}]]")
            else:
                methods.append(f"[[Coffee Preparation - {preparation.name}|{preparation.name}]]")
            ...
            zettel_note.set_frontmatter("coffeeToWaterRatio", f"{round(brew.brew_quantity/brew.grind_weight, 2)}ml/g")
            ...
            for note in items:
                if note["path"].is_file() is False:
                    ...
                    if attachment_value is not None:
                        ...
                        attachment_path = self.bean_conqueror_folder.joinpath(attachment_value)
                        if attachment_path.is_file():
                            new_image_name = note['note'].get_frontmatter("id") + ".webp"
                            new_path = note["path"].parent.joinpath(f"{new_image_name}")
                            self.compress_image(attachment_path, new_path)
                            note["note"].set_frontmatter("banner", f"![[{new_image_name}]]")
